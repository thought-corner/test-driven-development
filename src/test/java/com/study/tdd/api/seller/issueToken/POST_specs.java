package com.study.tdd.api.seller.issueToken;
...
/**
 * {@code POST /seller/issueToken} 엔드포인트의 명세(specification)를 검증하는 통합 테스트.
 *
 * <p>판매자 자격증명(이메일 + 비밀번호)을 검증하고 접근 토큰(access token)을 발행하는 계약을 다룬다.
 ...
 * </ul>
 *
 * <p>성공 케이스는 Arrange 단계에서 먼저 회원가입을 시켜 "존재하는 판매자"라는 선행 상태를 만든다.
